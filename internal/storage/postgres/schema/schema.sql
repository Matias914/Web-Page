...
-- Last modification date: 2025-10-26 23:30:11.29
...
CREATE TABLE celebrities (
    ...
    CONSTRAINT CHK_NAME CHECK (( LENGTH ( TRIM ( name ) ) > 0 )) NOT DEFERRABLE INITIALLY IMMEDIATE,
    CONSTRAINT AK_CELEBRITY UNIQUE (name, birth_date) NOT DEFERRABLE INITIALLY IMMEDIATE,
...
CREATE TABLE movies (
    ...
    CONSTRAINT AK_TITLE_RELEASE UNIQUE (title, released_at) NOT DEFERRABLE  INITIALLY IMMEDIATE,
    CONSTRAINT CHK_TITLE CHECK (( LENGTH ( TRIM ( title ) ) > 0 )) NOT DEFERRABLE INITIALLY IMMEDIATE,
    CONSTRAINT CHK_SYNOPSIS CHECK (( LENGTH ( TRIM ( synopsis ) ) > 0 )) NOT DEFERRABLE INITIALLY IMMEDIATE,
    CONSTRAINT CHK_DURATION CHECK (( duration_minutes > 0 )) NOT DEFERRABLE INITIALLY IMMEDIATE,
    CONSTRAINT CHK_POSTER CHECK (( poster_url IS NULL OR LENGTH ( TRIM ( poster_url ) ) > 0 )) NOT DEFERRABLE INITIALLY IMMEDIATE,
...
CREATE TABLE ratings (
    ...
    CONSTRAINT CHK_RATING CHECK (( rating between 1 and 10 )) NOT DEFERRABLE INITIALLY IMMEDIATE,
...
CREATE TABLE reviews (
    ...
    created_at timestamp  NOT NULL DEFAULT now(),
    CONSTRAINT CHK_COMMENT CHECK (( LENGTH ( TRIM ( comment ) ) > 0 )) NOT DEFERRABLE INITIALLY IMMEDIATE,
...
CREATE TABLE roles (
    ...
    CONSTRAINT CHK_ROLE CHECK (( LENGTH ( TRIM ( role ) ) > 0 )) NOT DEFERRABLE INITIALLY IMMEDIATE,
    CONSTRAINT PK_ROLES PRIMARY KEY (celebrity_id,movie_id)
);
...
CREATE TABLE users (
    ...
    CONSTRAINT CHK_USERNAME CHECK (( LENGTH ( TRIM ( username ) ) > 0 )) NOT DEFERRABLE INITIALLY IMMEDIATE,
    CONSTRAINT CHK_PASSWORD CHECK (( LENGTH ( TRIM ( password ) ) > 0 )) NOT DEFERRABLE INITIALLY IMMEDIATE,
    CONSTRAINT CHK_MAIL CHECK (( position ( '@' in mail ) > 1 )) NOT DEFERRABLE INITIALLY IMMEDIATE,
