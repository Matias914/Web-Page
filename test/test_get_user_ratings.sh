...
run_get_test "Obtención exitosa" "$APP_TEST_URL/api/users/$USER_ID/ratings?page=1&rows=5" 200

# Caso 2: ID de usuario no es un número
run_get_test "ID de usuario no es un número" "$APP_TEST_URL/api/users/abc/ratings?page=1&rows=5" 500

# Caso 3: Usuario no encontrado
run_get_test "Usuario no encontrado" "$APP_TEST_URL/api/users/999999/ratings?page=1&rows=5" 404

# Caso 4: Paginación inválida
run_get_test "Paginación inválida" "$APP_TEST_URL/api/users/$USER_ID/ratings?page=abc&rows=5" 500
