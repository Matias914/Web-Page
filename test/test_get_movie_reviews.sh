...
run_get_test "Obtención exitosa" "$APP_TEST_URL/api/movies/$MOVIE_ID/reviews?page=1&rows=5" 200

# Caso 2: ID de película no es un número
run_get_test "ID de película no es un número" "$APP_TEST_URL/api/movies/abc/reviews?page=1&rows=5" 500

# Caso 3: Película no encontrada
run_get_test "Película no encontrada" "$APP_TEST_URL/api/movies/999999/reviews?page=1&rows=5" 404

# Caso 4: Paginación inválida
run_get_test "Paginación inválida" "$APP_TEST_URL/api/movies/$MOVIE_ID/reviews?page=abc&rows=5" 500
