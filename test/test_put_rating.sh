...
run_put_test "Actualización exitosa" "$APP_TEST_URL/api/users/$USER_ID/ratings/$MOVIE_ID" '{"rating": 5}' 200

# Caso 2: Rating no encontrado
run_put_test "Rating no encontrado" "$APP_TEST_URL/api/users/$USER_ID/ratings/9999" '{"rating": 5}' 404

# Caso 3: Falla de validación (rating > 10)
run_put_test "Falla de validación" "$APP_TEST_URL/api/users/$USER_ID/ratings/$MOVIE_ID" '{"rating": 11}' 400
